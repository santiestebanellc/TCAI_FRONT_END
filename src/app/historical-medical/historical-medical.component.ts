...
export class HistoricalMedicalComponent implements OnInit {
  ...
  loadMedicalRecords() {
    this.http.get<any>('http://localhost:8000/diagnostico/paciente/1').subscribe({
      next: (response) => {
        if (response.success && response.content) {
          this.records = response.content.map((item: any) => {
            ...
            return {
              ...
              priority: (item.detalle_diagnostico.o2 || '').toString().includes('90') // Prioridad si el O2 contiene 90
            };
          });
